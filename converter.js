...
const audioAcceptTypes = `audio/*,${Array.from(audioExtensions)
  .map((ext) => `.${ext}`)
  .join(",")},.zip,application/zip`;
const videoAcceptTypes = `video/*,${Array.from(videoExtensions)
  .map((ext) => `.${ext}`)
  .join(",")},.zip,application/zip`;
...
const selectFiles = (files = []) => {
  state.selectedFiles = Array.from(files)
    .filter(Boolean)
    .filter((file) => isZipFile(file.name || "") || shouldIncludeFileForMode(file, currentMode));
  state.mediaEntries = [];
