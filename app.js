...
const singleDownloadLink = document.getElementById("download-link");
...
const resetSingleDownloadLink = () => {
  if (!singleDownloadLink) return;
  singleDownloadLink.hidden = true;
  singleDownloadLink.removeAttribute("href");
  singleDownloadLink.removeAttribute("download");
  singleDownloadLink.textContent = "";
};

const clearResults = () => {
  revokeObjectUrls();
  resultSummary.textContent = "";
  resetSingleDownloadLink();
...
const buildDownloadLinks = (results) => {
  ...
  if (results.length <= 3) {
    ...
    registerObjectUrl(url);
    const fileName = `extracted_audio_${Date.now()}.zip`;
    if (singleDownloadLink) {
      singleDownloadLink.hidden = false;
      singleDownloadLink.href = url;
      singleDownloadLink.download = fileName;
      singleDownloadLink.textContent = "下载音频打包文件";
      const sizeSpan = document.createElement("span");
      sizeSpan.className = "download-size";
      sizeSpan.textContent = formatBytes(zipBlob.size);
      singleDownloadLink.appendChild(sizeSpan);
    } else {
      const link = document.createElement("a");
      link.href = url;
